...
int DirectorioFunciones::insertar(const string& nombre, int tipo) {
    ...
    directorio[nombre] = move(f);
    return 0;
}

// busca una funcion por nombre, retorna un pointer a la Funcion
Funcion* DirectorioFunciones::buscar(const string& nombre) {
...
void DirectorioFunciones::imprimir() const {
    ...
    for (const auto& [nombre, fPtr] : directorio) {
        cout << "\n  Función : " << fPtr->nombre << "\n";
        cout << "  Retorno : " << tipoToString(fPtr->tipoRetorno) << "\n";
        cout << "  Params  : " << fPtr->numParams << "\n";
        cout << "  StartQ  : " << fPtr->startQuad << "\n";
        cout << "  Variables locales:\n";
...
void StackIDs::push(const string& nombre) {
...
string StackIDs::pop() {
    if (datos.empty()) {
        cerr << "StackIDs: underflow (pila vacía)\n";
        return "";
    }
    string val = datos.back();
    datos.pop_back();
    return val;
}
